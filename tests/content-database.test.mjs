...
test("keeps content cohort-scoped and protects referenced cohorts", { skip: !process.env.DATABASE_URL }, async () => {
  ...
  try {
    const first = await client.query("INSERT INTO cohorts (name) VALUES ($1) RETURNING id", [`Test A ${Date.now()}`]);
    const second = await client.query("INSERT INTO cohorts (name) VALUES ($1) RETURNING id", [`Test B ${Date.now()}`]);
    ...
    const created = await client.query("INSERT INTO lesson_summaries (cohort_id, major, title, description, image_url, resource_url) VALUES ($1, 'Computer Science', 'Visible', 'Test', 'https://example.com/a.png', 'https://example.com/a') RETURNING id", [first.rows[0].id]);
    const hidden = await client.query("INSERT INTO lesson_summaries (cohort_id, major, title, description, image_url, resource_url) VALUES ($1, 'Computer Science', 'Hidden', 'Test', 'https://example.com/b.png', 'https://example.com/b') RETURNING id", [second.rows[0].id]);

    const visible = await client.query("SELECT title FROM lesson_summaries WHERE cohort_id = $1 AND major = 'Computer Science'", [first.rows[0].id]);
