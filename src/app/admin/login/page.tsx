...
export default function AdminLoginPage() {
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-[#111b38]/95 p-6 shadow-[0_18px_60px_rgba(0,0,0,0.32)] sm:p-10">
      ...
      <form action={action} autoComplete="off" className="mt-8 space-y-5" aria-describedby="admin-status">
        <div>
          <label className="mb-2 block text-sm font-medium" htmlFor="username">Username</label>
          <input autoComplete="off" className="h-12 w-full rounded-xl bg-[#eef0f5] px-4 text-[#202338]" id="username" name="username" required />
        </div>
        <div>
          <label className="mb-2 block text-sm font-medium" htmlFor="password">Password</label>
          <input autoComplete="off" className="h-12 w-full rounded-xl bg-[#eef0f5] px-4 text-[#202338]" id="password" name="password" required type="password" />
        </div>
